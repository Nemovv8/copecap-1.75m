...
def load_data_for_inference_2(annot_path, caps_path=None):
    annotations = json.load(open(annot_path))['images']
    if caps_path is not None:
        retrieved_caps = json.load(open(caps_path))
    # data = {'restval': []}
    data = {'train': [], 'test': [], 'val': [], 'restval': []}

    for item in annotations:
        file_name = item['filename'].split('_')[-1]
        if caps_path is not None:
            # print("Checking image:", item['cocoid'], "| as str:", str(item['cocoid']))
            # print("retrieved_caps keys:", list(retrieved_caps.keys())[:5])  # 打印前5个键看格式
            # caps = retrieved_caps[str(item['cocoid'])]
            raw_caps = retrieved_caps.get(str(item['cocoid']), [])
            caps = [cap[0] if isinstance(cap, list) else cap for cap in raw_caps]
        else:
            caps = None
        image = {'file_name': file_name, 'caps': caps, 'image_id': str(item['cocoid'])}

        if item['split'] == 'restval':
            data['restval'].append(image)
