...
CAPTION_LENGTH = 25
SIMPLE_PREFIX = "What this image depicts is "

def prep_strings(text, tokenizer, template=None, retrieved_caps=None, summary_caps=None, k=None, is_test=False, max_length=None):
    ...
    # if summary_caps is not None:
    #     prefix = prefix.replace('**', summary_caps)
    # else:
    #     prefix = SIMPLE_PREFIX
    ...
    if template is not None:
        ...
        if summary_caps is not None:
            summary_text = summary_caps.strip()
            # print(f"[DEBUG] summary_caps: {summary_text}")
            if not summary_text.endswith('.'):
                summary_text += '.'
            prefix = prefix.replace('**', summary_text)
    else:
        prefix = SIMPLE_PREFIX
    ...
    def __getitem__(self, idx):
        ...
        if self.rag: 
            ...
            decoder_input_ids, labels = prep_strings(text, self.tokenizer, template=self.template,
                                                     retrieved_caps=caps, summary_caps=summary, k=self.k, max_length=self.max_target_length)
        else:
...
def load_data_for_training(annot_path, caps_path=None, sum_path=None):
    ...
    if sum_path is not None:
        summary_caps = json.load(open(sum_path))
    data = {'train': [], 'val': []}
    ...
    for item in annotations:
        ...
        if caps_path is not None:
            ...
            caps = None
        if sum_path is not None:
            summary = summary_caps[str(item['cocoid'])]['summary']
        else:
            summary = None
        samples = []
        for sentence in item['sentences']:
            samples.append({'file_name': file_name, 'cocoid': str(item['cocoid']), 'caps': caps, 'summary': summary, 'text': ' '.join(sentence['tokens'])})
...
def load_data_for_inference(annot_path, caps_path=None, sum_path=None):
    annotations = json.load(open(annot_path))['images']
    if caps_path is not None:
        retrieved_caps = json.load(open(caps_path))
    if sum_path is not None:
    ...
    for item in annotations:
        ...
        if sum_path is not None:
            ...
            summary = None
        image = {'file_name': file_name, 'caps': caps, 'summary': summary, 'image_id': str(item['cocoid'])}
        if item['split'] == 'test':
