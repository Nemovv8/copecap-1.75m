...
class ThisOPTAttention(OPTAttention):
    ...
    def __init__(
        ...
        super().__init__(config=config, is_cross_attention=is_cross_attention)
        ...
        # self.head_dim = int((self.embed_dim // self.num_heads) / self.cross_attention_reduce_factor)
        self.head_dim = embed_dim // num_heads
    ...
    def _shape(self, tensor: torch.Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2).reshape(bsz * self.num_heads, seq_len, self.head_dim)
...
class ThisOPTDecoder(OPTDecoder):
    ...
    # Copied from BartModel._prepare_decoder_attention_mask
    # Expand attention_mask to [batch_size, 1, tgt_len, tgt_len + past_len]
        combined_attention_mask = None

        if input_shape[-1] > 1:
        # Generate causal mask
            causal_mask = self._make_causal_mask(input_shape, inputs_embeds.dtype, device=inputs_embeds.device,
        ...
        if attention_mask is not None:
            ...
            combined_attention_mask = (
                expanded_attn_mask if causal_mask is None else expanded_attn_mask + causal_mask
            )
        else:
    ...
    def _expand_mask(self, mask: torch.Tensor, dtype: torch.dtype, tgt_len: Optional[int] = None):
        """
        Expands attention_mask from `[bsz, src_len]` to `[bsz, 1, tgt_len, src_len]`
        """
    ...
    def _make_causal_mask(self, input_shape, dtype, device, past_key_values_length=0):
        """
        Generates a causal mask of shape `[bsz, 1, tgt_len, src_len]`
        """
        bsz, tgt_len = input_shape
        mask = torch.full((tgt_len, tgt_len), float("-inf"), device=device)
        mask_cond = torch.arange(mask.size(-1), device=device)
        mask = mask.masked_fill(mask_cond[None, :] <= mask_cond[:, None], 0)
        mask = mask.to(dtype)

        if past_key_values_length > 0:
            past_mask = torch.zeros(tgt_len, past_key_values_length, dtype=dtype, device=device)
