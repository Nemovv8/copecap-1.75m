...
from src_new.utils_round2new import load_data_for_inference, prep_strings, postprocess_preds
...
def evaluate_rag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        summary = eval_df['summary'][idx]
        decoder_input_ids = prep_strings('', tokenizer, template=template, retrieved_caps=caps, summary_caps=summary, 
...
def main(args):
    ...
    data = load_data_for_inference(args.annotations_path, args.captions_path, args.summary_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--features_path", type=str, default='features1028/val.hdf5', help="H5 file with cached input image features")
    ...
    parser.add_argument("--decoder_name", type=str, default="facebook/opt-350m", help="Decoder name as found of HuggingFace or stored locally")
    ...
    parser.add_argument("--template_path", type=str, default="src_new/template2.txt", help="TXT file with template")
    parser.add_argument("--summary_path", type=str, default="data/summary_prompt_dataset.json", help="JSON file with summary generated in 1rd")
