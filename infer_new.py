...
from src.utils_new import load_data_for_inference, prep_strings, postprocess_preds  # ✅ 使用增强版 utils
...
def evaluate_norag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    for idx in tqdm(range(0, len(eval_df), bs)):
        ...
        summary_prompts = eval_df.get('summary_prompt', [None]*len(image_ids))
        decoder_input_ids = [prep_strings('', tokenizer, is_test=True, summary_prompt=summary_prompts[i]) for i in range(len(image_ids))] 
...
def evaluate_rag_model(args, feature_extractor, tokenizer, model, eval_df):
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        summary_prompt = eval_df.get('summary_prompt', [None]*len(eval_df))[idx]
        decoder_input_ids = prep_strings('', tokenizer, template=template, retrieved_caps=caps,
                                         k=int(args.k), is_test=True, summary_prompt=summary_prompt)

        if args.features_path is not None:
            encoder_last_hidden_state = torch.FloatTensor([features[image_id][()]]).to(args.device)
...
def main(args):
    ...
    data = load_data_for_inference(args.annotations_path, args.captions_path, args.summary_prompt_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--summary_prompt_path", type=str, default="data/generated_summary_prompts.json", help="Path to summary prompt JSON")
    ...
    parser.add_argument("--encoder_name", type=str, default="openai/clip-vit-large-patch14", help="Encoder name as found of HuggingFace or stored locally")
    parser.add_argument("--decoder_name", type=str, default="gpt2-medium", help="Decoder name as found of HuggingFace or stored locally")
